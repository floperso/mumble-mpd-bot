CFLAGS = `pkg-config --libs --cflags libssl lua libprotobuf-c opus vorbis vorbisfile` -pthread -I/usr/include/lua5.2
LDFLAGS := -L/usr/lib -L/usr/local/lib/ -ldl -lprotobuf-c -lev -lprotobuf -llua5.2 -lssl -lopus -llua5.2-socket $(LDFLAGS)
...
piepan: src/piepan.c src/piepan.h src/util.c src/events.c src/handlers.c \
	...
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ src/piepan.c proto/Mumble.o
...
src/piepan_impl.lua: $(LUAFILES)
	echo 'local socket = require("socket.core")\n' > $@
	cat $(LUAFILES) >> $@
